...
logger "Start swap control service.";
sudo tail -1 /var/log/messages >> ~/swap-control.log
...
finish() {
	echo "Service swap control finished."
	logger "Service swap control finished."
	sudo tail -1 /var/log/messages >> ~/swap-control.log
	exit 0
}
...
while true; do
	...
	if (( result == 1 )); then
		create_new_swap "$k" ;
		let "k += 1";
		logger "Create new file for swap. Doubled swap space."
		sudo tail -1 /var/log/messages >> ~/swap-control.log
	fi
	#sleep 30 &
	#wait
